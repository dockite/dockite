...
export const RestoreDocumentPage = defineComponent({
  ...
  setup: () => {
    ...
    const handleRestoreDocument = usePromiseLazy(async () => {
      try {
        if (documentId.value && deletedDocument.result.value) {
          const result = await restoreDocument(deletedDocument.result.value);
    ...
    return () => {
      return (
        <div v-loading={handleRestoreDocument.loading.value} class="relative">
          ...
          <RenderIfComponent condition={deletedDocument.result.value !== null}>
            ...
            <div>
              <h2 class="text-xl font-semibold pb-5">
                Are you sure you want to restore {documentIdentifier.value}?
              </h2>

              <p class="pb-5">
                Restoring {documentIdentifier.value} means that it will become accessible within the
                API once again.
              </p>
...
export default RestoreDocumentPage;
