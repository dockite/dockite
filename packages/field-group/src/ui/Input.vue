            ...
            <template v-if="repeatable && Array.isArray(fieldData)">
              ...
              >
                <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                  ...
                  >
                    <div
                      class="flex flex-col justify-center items-center px-3 text-xl dockite--group-movement-actions relative"
                    >
                      <el-button
                        v-if="repeatable"
...
export default class GroupFieldInputComponent extends Vue {
  ...
  public initialiseFormData(): void {
    ...
    if (this.repeatable) {
      ...
      if (Array.isArray(this.value)) {
        this.fieldData = this.value.map(v => ({ ...cloneDeep(this.initialFieldData), ...v }));
      }
    } else {
      this.fieldData = { ...cloneDeep(this.initialFieldData), ...this.value };
    }
